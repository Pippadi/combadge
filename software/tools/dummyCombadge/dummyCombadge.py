...
if len(sys.argv) < 4:
    print(
        "USAGE: dummyCombadge.py <Raw Sound File To Send> <Raw Sound File To Receive> <Remote Combadge/Bridge>"
    )
    exit(1)
...
def sendData():
    ...
    for i in range(len(outgoing) // (BUF_LEN * BYTES_PER_SAMPLE) + 1):
        ...
        conn.sendall(
            Header(CRAPMsgType.AUDIO_DATA, len(chunk)).asBytes() + bytes(chunk)
        )
